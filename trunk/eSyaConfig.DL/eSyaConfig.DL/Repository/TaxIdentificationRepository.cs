            ...
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        var _isTaxstatecodeExist = db.GtEccnti.Where(w => w.Isdcode == obj.Isdcode && w.StateCode.ToUpper().Replace(" ", "") == obj.StateCode.ToUpper().Replace(" ", "")).Count();
                        if (_isTaxstatecodeExist > 0)
                        {
                            return new DO_ReturnParameter() { Status = false, Message = "State Code is already Exists For Selected ISD Code & Tax Code" };
                        }
            ...
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        GtEccnti statecode = db.GtEccnti.Where(w => w.Isdcode == obj.Isdcode && w.StateCode.ToUpper().Replace(" ", "") == obj.StateCode.ToUpper().Replace(" ", "")
                         && w.TaxIdentificationId != obj.TaxIdentificationId).FirstOrDefault();
                        if (statecode != null)
                        {
                            return new DO_ReturnParameter() { Status = false, Message = "State Code is already Exist For Selected ISD Code." };
                        }
