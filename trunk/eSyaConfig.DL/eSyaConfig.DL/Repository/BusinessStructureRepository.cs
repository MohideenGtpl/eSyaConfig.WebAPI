   ...
   public class BusinessStructureRepository:IBusinessStructureRepository
    {
        ...
        public async Task<bool> IsAllowMultiSegment(int BusinessId)
        {
            using (var db = new eSyaEnterprise())
            {
                try
                {
                    ...
                    //else
                    //    return true;
                    var SS = await db.GtEcbsen.Where(x => x.BusinessId == BusinessId && x.BusinessUnitType == "M" && x.ActiveStatus == true).CountAsync();
            ...
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        GtEcbsen entity = db.GtEcbsen.Where(x => x.BusinessId == BusinessSegment.BusinessId && x.ActiveStatus == true).FirstOrDefault();

                        if (entity != null)
                        {
                            int noOfSegment = db.GtEcbssg.Where(s => s.BusinessId == entity.BusinessId && s.ActiveStatus).Count();
                            if(noOfSegment >= entity.NoOfUnits)
                            {
                                return new DO_ReturnParameter() { Status = false, Message = "Entity Allowed only "+ noOfSegment.ToString() + " Segment." };
                            }
        ...
        public async Task<List<DO_BusinessLocation>> GetBusinessLocationByBusinessId(int BusinessId)
        {
            try
            {
                using (var db = new eSyaEnterprise())
                {
                    var locs = db.GtEcbsln.Where(x => x.BusinessId == BusinessId)
                        .Join(db.GtEccuco,
                         x => x.CurrencyCode,
                         y => y.CurrencyCode,
            ...
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        GtEcbsln exists = db.GtEcbsln.Where(x => x.BusinessId == obj.BusinessId && x.SegmentId == obj.SegmentId).FirstOrDefault();
                        if (exists != null)
                        {
                            return new DO_ReturnParameter() { Status = false, Message = "Business Code and Segment code already Exists" };

                        }
                        
                        GtEcbsln is_LocationCodeExists = db.GtEcbsln.FirstOrDefault(l => l.LocationCode.ToUpper().Replace(" ", "") == obj.LocationCode.ToUpper().Replace(" ", "")
                        ...
                        GtEcbsln is_locDescExists = db.GtEcbsln.FirstOrDefault(l => l.LocationDescription.ToUpper().Replace(" ", "") == obj.LocationDescription.ToUpper().Replace(" ", "")
                        && l.BusinessId == obj.BusinessId );
                        ...
                        int Business_Key = Convert.ToInt32(obj.BusinessId.ToString() + obj.SegmentId.ToString());
                        var is_BusinessKeyExist = db.GtEcbsln.Where(x => x.ActiveStatus == true && x.BusinessKey == Business_Key).FirstOrDefault();
                        if (is_BusinessKeyExist != null)
                        {
                            return new DO_ReturnParameter() { Status = false, Message = "Business Key already Exists for selected Entity and Segment" };
                        }
            ...
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        GtEcbsln is_locDescExists = db.GtEcbsln.FirstOrDefault(l => l.LocationDescription.ToUpper().Replace(" ", "") == obj.LocationDescription.ToUpper().Replace(" ", "") &&
                        l.BusinessId == obj.BusinessId && l.SegmentId != obj.SegmentId);
                        if (is_locDescExists != null)
                        {
                            return new DO_ReturnParameter() { Status = false, Message = "Business Location already Exists for selected Entity and Segment." };
                        ...
                        GtEcbsln is_LocCodeExists = db.GtEcbsln.FirstOrDefault(l => l.LocationCode.ToUpper().Replace(" ", "") == obj.LocationCode.ToUpper().Replace(" ", "")
                        && l.BusinessId == obj.BusinessId && l.SegmentId != obj.SegmentId);
                        if (is_LocCodeExists != null)
                        {
                            return new DO_ReturnParameter() { Status = false, Message = "Location Code already Exists for selected Entity and Segment." };
                        ...
                        GtEcbsln b_loc = db.GtEcbsln.Where(bl => bl.BusinessId == obj.BusinessId && bl.SegmentId == obj.SegmentId).FirstOrDefault();

                        if (b_loc != null)
                        {
                            b_loc.LocationCode = obj.LocationCode;
        ...
        public async Task<DO_ReturnParameter> ActiveOrDeActiveBusinessLocation(bool status, int BusinessId,int SegmentId)
        {
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        GtEcbsln b_loc = db.GtEcbsln.Where(bl => bl.BusinessId == BusinessId && bl.SegmentId == SegmentId).FirstOrDefault();
                        if (b_loc == null)
                        {
                            return new DO_ReturnParameter() { Status = false, Message = "Business Location not found." };
                        }
                        ...
                        if (status == true)
                            return new DO_ReturnParameter() { Status = true, Message = "Business Location Activated Successfully." };
                        else
                            return new DO_ReturnParameter() { Status = true, Message = "Business Location De Activated Successfully." };
                    }
                    catch (DbUpdateException ex)
                    {
                        dbContext.Rollback();
        ...
        public async Task<List<DO_TaxIdentification>> GetTaxIdentificationByISDCode(int ISDCode)
        {
            try
            {
                using (var db = new eSyaEnterprise())
                {
                    var ds = db.GtEccnti.Where(w => w.Isdcode == ISDCode && w.ActiveStatus==true)
        ...
        public async Task<List<DO_CountryCodes>>  GetCurrencyListbyIsdCode(int IsdCode)
        {
            try
            {
                using (var db = new eSyaEnterprise())
                {
                    var Currency = db.GtEccncd.Where(c => c.Isdcode == IsdCode && c.ActiveStatus).Join(db.GtEccuco,
                         x => x.CurrencyCode,
                         y => y.CurrencyCode,
        ...
        public async Task<List<DO_BusinessLocation>> GetActiveLocationsAsSegments()
        {
            try
            {
                using (var db = new eSyaEnterprise())
                {
                    var ds = db.GtEcbsln.Where(w =>w.ActiveStatus == true && w.IsBookOfAccounts==true)
        ...
        public async Task<List<DO_Place>> GetCityListbyISDCode(int isdCode)
        {
            using (var db = new eSyaEnterprise())
            {
                var pf = db.GtAddrin
                .Join(db.GtEccnpl.Where(w => w.PlaceType == PlaceTypeValues.District && w.ActiveStatus && w.Isdcode==isdCode),
                    a => new { a.District },
                    c => new { District = c.PlaceId },
                    (a, c) => new { a, c })
               .Where(w => w.a.ActiveStatus && w.a.Isdcode==isdCode)
        ...
        public async Task<DO_TaxIdentification> GetStateCodeByISDCode(int isdCode,int TaxIdentificationId)
        {
            try
            {
                using (var db = new eSyaEnterprise())
                {
                    var ds = db.GtEccnti.Where(w => w.Isdcode == isdCode && w.TaxIdentificationId== TaxIdentificationId && w.ActiveStatus)
                        .Select(x => new DO_TaxIdentification
                        {
                            TaxIdentificationId = x.TaxIdentificationId,
                            TaxIdentificationDesc = x.TaxIdentificationDesc,
        ...
        public async Task<List<DO_BusienssSegmentCurrency>> GetCurrencybyBusinessKey(int BusinessKey)
        {
            try
            {
                using (var db = new eSyaEnterprise())
                {
                    ...
                    foreach (var obj in ds)
                    {
                        GtEcbssc sbCurrency = db.GtEcbssc.Where(x => x.BusinessKey == BusinessKey && x.TocurrencyCode.ToUpper().Replace(" ", "") == obj.CurrencyCode.ToUpper().Replace(" ", "")).FirstOrDefault();
                        if (sbCurrency != null)
                        {
                            obj.ActiveStatus = sbCurrency.ActiveStatus;
                        }
