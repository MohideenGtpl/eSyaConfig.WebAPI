...
namespace HCP.Config.DL.Repository
{
    public class DocumentControlRepository:IDocumentControlRepository
    {
        ...
        public async Task<List<DO_FormNames>> GetDocumentRequiredForms(int paramId)
        {
            try
            {
                using (var db = new eSyaEnterprise())
                {
                    ...
                    var ds =await db.GtEcfmpa.Where(x => x.ActiveStatus == true && x.ParameterId == paramId).Join(db.GtEcfmfd,
                        d => d.FormId,
                        f => f.FormId,
                        (d, f) => 
                        new DO_FormNames
                    ...
                    var distinctList = ds.GroupBy(e => e.FormID).Select(g => g.First());
